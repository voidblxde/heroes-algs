...
**Особенности реализации (для корректной работы UI/игры):**
- имена создаются в формате `"Type N"` (с пробелом), чтобы UI корректно отображал и не падал;
- координаты юнитов в пресете выставляются в “зоне предпросмотра” (x=0..2), так как генерация вызывается на экране создания армии.
